...
def send_heartbeat():
    while True:
        response = requests.put(f"{CONSUL_AGENT}/v1/agent/check/pass/service:my-test-service")
        if response.status_code != 200:
            # Deregister the service if the heartbeat fails
            requests.put(f"{CONSUL_AGENT}/v1/agent/service/deregister/my-test-service")
